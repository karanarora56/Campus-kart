...
export const Explore = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        // If 'All' is selected, fetch everything. Otherwise, pass the category.
        ...
        // Map backend database fields to the props our ProductCard expects
        const formattedProducts = data.products.map(p => ({
          ...
          category: p.category,
          // Grab the first Cloudinary image, or use a fallback if something went wrong
    ...
    fetchProducts();
  }, [activeCategory]); // Re-run this fetch whenever the user clicks a new category pill
    ...
    <div className="w-full pb-10">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 py-20 dark:border-white/10">
          ...
          <p className="text-sm text-slate-400 dark:text-gray-500 mt-2">Try selecting a different category.</p>
        </div>
      ) : (
        <div className="columns-1 gap-4 sm:columns-2 md:columns-3 lg:columns-4">
