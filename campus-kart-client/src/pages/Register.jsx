...
import { Mail, Lock, User, Loader2, GraduationCap, ArrowLeft } from 'lucide-react'; // ADDED ArrowLeft
...
export const Register = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await register({ fullName, email, password, batch });
    } catch {
      ...
      <div className="w-full max-w-md space-y-8 rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
        <div>
          ...
          <p className="mt-3 text-center text-sm text-gray-400">
            Create your exclusive student account
          </p>
        </div>
          ...
          <div className="space-y-5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="block w-full rounded-2xl border border-white/10 bg-black/40 py-4 pl-12 pr-4 text-white placeholder-gray-500 transition-colors focus:border-electric-violet focus:outline-none focus:ring-1 focus:ring-electric-violet"
                placeholder="Graduation Batch (e.g., 2027)"
              />
            </div>
        ...
        <p className="text-center text-sm text-gray-400">
          ...
          <Link to="/login" className="font-semibold text-electric-violet transition-colors hover:text-white">
            Sign in here
          </Link>
        </p>
      </div>
