...
import { Package, Leaf, ShieldCheck, Loader2, CheckCircle2, Trash2, Heart, Search } from 'lucide-react';
...
export const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        // Fetch both "My Items" and "Saved Items" simultaneously for speed!
        const [myItemsRes, savedItemsRes] = await Promise.all([
          axiosInstance.get('/products/me'),
          axiosInstance.get('/products/saved/me')
        ]);
        
        setMyItems(myItemsRes.data.products);
        setSavedItems(savedItemsRes.data.products);
      } catch (error) {
  ...
  const markAsSold = async (productId) => {
    try {
      ...
      setMyItems(myItems.map(item => item._id === productId ? { ...item, status: 'Sold' } : item));
  ...
  const displaySavedItems = currentUser?.savedItems ? savedItems.filter(item => currentUser.savedItems.includes(item._id || item.id)) : savedItems;
  return (
    <div className="mx-auto max-w-6xl pb-20">
      
      {/* PROFILE HEADER */}
      <div className="mb-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-[#0B0E14]">
      ...
      {/* DASHBOARD TABS */}
      <div className="mb-8 flex gap-6 border-b border-slate-200 pb-px dark:border-white/10">
        <button 
          onClick={() => setActiveTab('listings')}
          className={`flex items-center gap-2 border-b-2 px-2 py-3 font-bold transition-all ${
            activeTab === 'listings' ? 'border-electric-violet text-electric-violet' : 'border-transparent text-slate-500 hover:text-slate-800 dark:text-gray-400 dark:hover:text-white'
          }`}
        >
          <Package size={20} /> My Campus Listings
        </button>
        <button 
          onClick={() => setActiveTab('saved')}
          className={`flex items-center gap-2 border-b-2 px-2 py-3 font-bold transition-all ${
            activeTab === 'saved' ? 'border-rose-500 text-rose-500' : 'border-transparent text-slate-500 hover:text-slate-800 dark:text-gray-400 dark:hover:text-white'
          }`}
        >
          <Heart size={20} /> Saved Items ({displaySavedItems.length})
        </button>
      </div>
      ...
      {loading ? (
        <div className="flex h-40 items-center justify-center">
          <Loader2 className={`animate-spin ${activeTab === 'listings' ? 'text-electric-violet' : 'text-rose-500'}`} size={40} />
        </div>
      ) : activeTab === 'listings' ? (
               ...
               <div className="flex flex-1 flex-col p-5">
                  ...
                  <div className="mt-auto pt-4 flex gap-2">
                    ...
                    >
                      ...
                      {item.status === 'Sold' ? (item.postType === 'Listing' ? 'Sold Out' : 'Resolved') : (item.postType === 'Listing' ? 'Mark Sold' : 'Resolve')}
                    </button>
                    <button 
        ...
        /* --- TAB 2: SAVED ITEMS (WISHLIST) --- */
        displaySavedItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 py-20 dark:border-white/10">
            <Heart size={48} className="mb-4 text-slate-300 dark:text-white/20" />
            <h3 className="text-xl font-bold text-slate-500 dark:text-gray-400">Your wishlist is empty</h3>
            <p className="text-sm text-slate-400 dark:text-gray-500 mt-2">Click the heart icon on any post to save it for later.</p>
            <Link to="/explore" className="mt-6 rounded-xl bg-electric-violet px-6 py-3 font-bold text-white transition-all hover:bg-[#6D28D9]">
              Start Exploring
            </Link>
          </div>
        ) : (
